...
8/6/2019 - Note: removed sigma_en_sys term (in ej_228_uncerts) because it is encapulated in detector positional uncertainty
         - updated to work with plot_acp_lo_curves function in lo_analysis.py
'''
import numpy as np
...
def calc_ep_uncerts(counts, angles, dists,  beam_11MeV, print_unc):
    # measurements
    #       BL 70                                                    BR 70
    ...
    sigma_sys = [En*np.sin(np.deg2rad(2*theta))*sigma_pos_degree[i] for i, theta in enumerate(angles)]
...
if __name__ == '__main__':
    ...
    angles = [70., 60., 50., 40., 30., 20., 20., 30., 40., 50., 60., 70.]
    dists = [66.4, 63.8, 63.2, 63.6, 64.9, 65.8, 66.0, 65.3, 63.4, 62.7, 64.3, 66.5]
    calc_ep_uncerts([100]*12, angles, dists, beam_11MeV=True, print_unc=True)
    calc_ep_uncerts([100]*12, angles, dists, beam_11MeV=False, print_unc=True)