...
det_size = [np.rad2deg(np.arctan(2.54/d)) for d in dists]

sigma_ang = [np.sqrt(axes_marks**2 + housing_mount**2 + d**2) for d in det_size]

# total uncert
sigma_tot = [np.sqrt(sigma_pos**2 + s**2) for s in sigma_ang]
En_11 = 11.325 # MeV
Ep_11 = [En_11 * np.sin(np.deg2rad(a))**2 for a in angles]
Ep_11_uncert = [En_11 * np.sin(np.deg2rad(a + sigma_tot[i]))**2 - Ep_11[i] for i, a in enumerate(angles)]
En_4 = 4.825
Ep_4 = [En_4 * np.sin(np.deg2rad(a))**2 for a in angles]
Ep_4_uncert = [En_4 * np.sin(np.deg2rad(a + sigma_tot[i]))**2 - Ep_4[i] for i, a in enumerate(angles)]


print '   angle (deg) |  uncert (deg) | uncert 11.3 (MeV) | rel uncert | uncert 4.8 (MeV) | rel uncert'
for i, sigma in enumerate(sigma_tot):
    if i == 0:
